...
public class DataBaseSQL extends SQLiteOpenHelper {
    ...
    //CRUD CUIDADORES------------------------------------------------------------------------------
    //INSERT CUIDADOR
    public boolean insertarCuidador(String nombre, String apellidos, String dni, String telefono, String fechaNacimiento, String sexo, String nombreUsuario, String contraseniaUsuario, String correo, String nacionalidad, String direccion,String experiencia, String disponibilidad) {
        SQLiteDatabase db = this.getWritableDatabase();
        db.execSQL("INSERT INTO cuidadores VALUES (null, '" + nombre + "', '" + apellidos + "', '"+dni+"', '"+telefono+"', '"+fechaNacimiento+"', '"+sexo+"', '"+nombreUsuario + "', '"+contraseniaUsuario+"', '"+correo+"', '"+nacionalidad+"', '"+direccion+"', '"+experiencia+"', '"+disponibilidad+"')");
        return true;
    }
    ...
    public ArrayList<String> consultarCuidadores() {
        ...
        if (cantidadResultados > 0)
        {
            ...
            while(res.isAfterLast()==false)
            {
                contenido = res.getString(res.getColumnIndex("nombre")) + ".-" + res.getString(res.getColumnIndex("apellidos"))
                        + ".-" + res.getString(res.getColumnIndex("sexo")) + ".-" + res.getString(res.getColumnIndex("experiencia"))
                        + ".-" + res.getString(res.getColumnIndex("disponibilidad")) + ".-" + res.getInt(res.getColumnIndex("puntuacion"));
                filas.add(contenido);
                res.moveToNext();
            }
        }
